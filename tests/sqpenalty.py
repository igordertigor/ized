...
def test_QR(compress):
    ...
    We do this my making sure that recovered parameters and residuals are
    independent of weather the data are observed in one go or in multiple
    chunks.
    """
    np.random.seed(0)
...
def test_consistency_with_fixed_regularizer():
    """As we collect more data, the estimated parameters should approximate
    generating values"""
    np.random.seed(0)
    b = np.ones(4, 'd')
...
def test_function_approximation(basis):
    """Using more data with the spline bases should actually result in better
    function estimates"""
    ...
    assert (np.sqrt(np.trapz((y2-ytrue)**2, x2)) <
            np.sqrt(np.trapz((y1-ytrue)**2, x2)))


def test_constraints(basis):
    """The constraint transformation should generate a matrix Z such that
    C*Z=0
    """
    x = np.random.rand(100, 1)
...
def test_multiple_penalties(basis):
    """For multiple penalties we want to make sure that the gcv for the final
    penalty is better than what we find through a coarse grid search.
    """
    np.random.seed(0)
    x = np.random.rand(40, 2)
    y = (np.cos(4*x[:, 0]) + (x[:, 0]-.5)**2 -
         x[:, 1]**3 + 1./(1+x[:, 1])
